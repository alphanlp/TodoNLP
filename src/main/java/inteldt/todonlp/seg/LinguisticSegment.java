	...
	public WordNet generateWordNet(final WordNet wordnet){
		...
		for (int index = 0; index < wordnet.sentence.length(); index++) {
			...
			while (index + pos <= wordnet.sentence.length()) {
				...
				//查询是否在词典中
				CoreDictionary.Attribute attri = CoreDictionary.trie.search(candidate);
				//加入词网
				if (attri != null) {// index+1，因为0的位置被开始节点占领
					wordnet.add(index + 1,new Vertex(candidate,candidate,attri));
				} else {
					if (pos == 1) {
						wordnet.add(index + 1, new Vertex(candidate,candidate,new CoreDictionary.Attribute(Nature.un,1)));
					}
				}
				pos++;
			}
